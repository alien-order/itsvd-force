...
| `bizNmDept` | `biznmdept` | 업무유형 | 상세정보 (대소문자 주의) |
...
> **주의**: `bizNmDept`는 API 원본 키가 대소문자 혼합. DB 저장 시 소문자 `biznmdept`로 저장됨.  
> 화면 표시: `vocInfo.bizNmDept` → DB에서 읽을 때는 SQLite가 case-insensitive하므로 동작.
...
| `vocstatuscode` | `vocstatuscode` | 단계상태코드 | API 원본 키 그대로 사용. type_items voc_status 자동 매핑에 사용 |
| `vocstatusname` | `vocstatusname` | 단계상태명 | API 원본 키 그대로 사용 |
...
fetch_voc(voc_number)
  └─ API 응답 파싱
       ├─ 기본 데이터 → vocs 테이블 (title, content, voc_number, due_date, requester, category, process_type)
       ├─ 전체 필드   → voc_info 테이블 (save_voc_info: API 키 그대로 컬럼으로 저장)
       └─ vocInfoList → voc_stage_info 테이블 (save_voc_stages: stage_index, stage_status, stage_data)
```
